...
import { ChatCompanion } from "./companions/ChatCompanion";
...
export async function createChat(situation: string = "water-cooler"): Promise<Drama> {
  const drama = await Drama.initialize(situation, [defaultCompanionConfig]);
  return drama;
}

export type { ChatCompanion };
export { defaultCompanionConfig };
